#!/usr/bin/env python3
"""
Generate the 6-panel pipeline visualization for ALL 5 objects in T-Less frame 10.
Each image shows: Mask → Shape Analysis → DT heatmap → Moments+Safety → Final → Full Frame
All values are computed live from real data.
"""
import cv2
import numpy as np
import os
...
def generate_object_pipeline(rgb, depth_mm, masks, obj_idx, out_path):
    ...
    crop_depth = depth_mm[y_min_c:y_max_c, x_min_c:x_max_c]
    ...
    # Global coordinates of final point
    ...
    canvas_h = panel_h * 2 + 150
    img = np.full((canvas_h, canvas_w, 3), BG, dtype=np.uint8)

    title = f"FULL PIPELINE: T-Less Frame 10, Object #{obj_idx} ({shape_label})"
    put_text_center(img, title, 25, 0.5, YELLOW, 2)
    ...
    p1 = crop_rgb.copy()
    p1[crop_mask > 0] = (p1[crop_mask > 0] * 0.5 + np.array(OBJ_COLORS[obj_idx % 5]) * 0.5).astype(np.uint8)
    ...
    if is_circular:
        # For circular: just show DT center directly
        cv2.circle(p4, (dtx, dty), 7, GREEN, -1)
        cv2.circle(p4, (dtx, dty), 7, WHITE, 2)
        put_text(p4, f"DT center", (dtx + 8, dty - 5), 0.3, GREEN)
        put_text(p4, f"({dtx},{dty})", (dtx + 8, dty + 10), 0.25, GREEN)
        # Also show moments for comparison
        cv2.circle(p4, (mx_i, my_i), 4, ORANGE, -1)
        put_text(p4, f"moments", (mx_i - 35, my_i - 8), 0.25, ORANGE)
        dist_dm = np.sqrt((dtx - mx_i)**2 + (dty - my_i)**2)
        if dist_dm > 3:
            cv2.line(p4, (mx_i, my_i), (dtx, dty), GRAY, 1)
            mid_x = (mx_i + dtx) // 2
            mid_y = (my_i + dty) // 2
            put_text(p4, f"{dist_dm:.0f}px", (mid_x, mid_y - 5), 0.25, YELLOW)
    else:
        # For non-circular: show moments center with clearance circle
    ...
    for mi, m in enumerate(masks):
        ...
        if np.sum(mb) < 100:
            continue
        color = OBJ_COLORS[mi % 5]
        p6[mb] = (p6[mb] * 0.5 + np.array(color) * 0.5).astype(np.uint8)

    # Highlight this object
    for c_full in contours_full:
        cv2.drawContours(p6, [c_full], -1, YELLOW, 2)

    # Draw the final center on full image
    ...
    return {
        ...
        'method': method_name,
        'safety': safety_result if not is_circular else "N/A",
...
def main():
    ...
    for obj_idx in range(len(masks)):
        ...
        if np.sum(mask > 0) < 100:
            ...
            continue
        out_path = os.path.join(OUT, f"06_tless_pipeline_obj{obj_idx}.png")
        result = generate_object_pipeline(rgb, depth_mm, masks, obj_idx, out_path)
        results.append(result)

    # Print summary
    print(f"\n{'='*70}")
    print(f"  SUMMARY: All Objects in Frame 10")
    print(f"{'='*70}")
    print(f"  {'Obj':>3} | {'Area':>7} | {'Circ':>5} | {'AR':>4} | {'Sol':>5} | {'Circular?':>9} | {'Method':>10} | {'Safety':>30} | {'Center':>12}")
    print(f"  {'-'*3}-+-{'-'*7}-+-{'-'*5}-+-{'-'*4}-+-{'-'*5}-+-{'-'*9}-+-{'-'*10}-+-{'-'*30}-+-{'-'*12}")
    for r in results:
        circ_str = "YES" if r['is_circular'] else "NO"
        print(f"  #{r['obj_idx']:>2} | {r['area']:>7} | {r['circularity']:>5.3f} | {r['aspect_ratio']:>4.2f} | {r['solidity']:>5.3f} | {circ_str:>9} | {r['method']:>10} | {r['safety']:>30} | ({r['center'][0]:>3}, {r['center'][1]:>3})")
