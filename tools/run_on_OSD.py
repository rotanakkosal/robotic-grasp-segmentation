...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="path to a single image file (overrides --dataset-path)"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--use-dummy-depth",
        action="store_true",
        help="Use dummy depth image (zeros) instead of reading from file"
    )
    return parser
...
if __name__ == "__main__":
    ...
    # Check if a single image path is provided
    if args.image_path is not None:
        ...
        # Load images - check for image_color subdirectory first, then direct directory
        rgb_paths = sorted(glob.glob("{}/image_color/*.png".format(args.dataset_path)) + 
                           glob.glob("{}/image_color/*.jpg".format(args.dataset_path)) +
                           glob.glob("{}/image_color/*.jpeg".format(args.dataset_path)) +
                           glob.glob("{}/image_color/*.HEIC".format(args.dataset_path)) +
                           glob.glob("{}/image_color/*.heic".format(args.dataset_path)))
        
        # If no images in image_color, check the directory directly
        if len(rgb_paths) == 0:
            rgb_paths = sorted(glob.glob("{}/*.png".format(args.dataset_path)) + 
                              glob.glob("{}/*.jpg".format(args.dataset_path)) +
                              glob.glob("{}/*.jpeg".format(args.dataset_path)) +
                              glob.glob("{}/*.HEIC".format(args.dataset_path)) +
                              glob.glob("{}/*.heic".format(args.dataset_path)))
    ...
    if args.use_dummy_depth:
        print("Using dummy depth images (zeros)")
        depth_paths = rgb_paths  # Just for iteration, won't be used
    else:
        depth_paths = sorted(glob.glob("{}/disparity/*.png".format(args.dataset_path)))
        if len(depth_paths) == 0:
            print("Warning: No depth images found. Using dummy depth images.")
            args.use_dummy_depth = True

    for idx, rgb_path in enumerate(rgb_paths):
        # Handle different image formats
        if rgb_path.lower().endswith(('.heic', '.heif')):
            ...
            try:
                ...
                print(f"Warning: Could not read HEIC file {rgb_path}: {e}")
                ...
                try:
                    ...
                    print(f"Error reading {rgb_path}: {e2}")
                    continue
        else:
        ...
        if args.use_dummy_depth:
            # Create dummy depth image (2D array, will be normalized to 3D)
            # Using a constant value in the middle of typical depth range (in mm)
            depth_img = np.ones((H, W), dtype=np.float32) * 800.0  # 800mm typical depth
            depth_img = normalize_depth(depth_img)  # Converts to [H, W, 3] uint8
        else:
            ...
            if depth_path and os.path.exists(depth_path):
                ...
                # Fallback to dummy depth if file doesn't exist
                depth_img = np.ones((H, W), dtype=np.float32) * 800.0
                depth_img = normalize_depth(depth_img)
        ...
        vis_all_img = np.hstack([rgb_img, vis_img])  # Only original RGB and segmentation
        ...
        output_filename = os.path.join(args.output_dir, f"{base_name}.png")
        cv2.imwrite(output_filename, vis_all_img)
